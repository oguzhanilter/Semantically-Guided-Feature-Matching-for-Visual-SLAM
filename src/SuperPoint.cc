...
cv::Mat SPdetect(std::shared_ptr<torch::jit::script::Module> model, cv::Mat img, std::vector<cv::KeyPoint> &keypoints, double threshold, bool nms, bool cuda)
{
    ...
    // auto prob = out->elements()[0].toTensor().to(torch::kCPU).squeeze(0);  // [H, W]
    // auto desc = out->elements()[1].toTensor().to(torch::kCPU);             // [1, 256, H/8, W/8]

    auto prob = out->elements()[0].toTensor().to(torch::kCPU);  // [H, W]
    auto desc = out->elements()[1].toTensor().to(torch::kCPU);             // [1, 256, H/8, W/8]
...
void SPDetector::detect(cv::Mat &img, bool cuda)
{
    ...
    // mProb = out->elements()[0].toTensor().to(torch::kCPU).squeeze(0);  // [H, W]
    // mDesc = out->elements()[1].toTensor().to(torch::kCPU);  // [1, 256, H/8, W/8]

    auto semi = out->elements()[0].toTensor().to(torch::kCPU);  // [H, W]
    auto desc = out->elements()[1].toTensor().to(torch::kCPU);  // [1, 256, H/8, W/8]
...
void SPDetector::computeDescriptors(const std::vector<cv::KeyPoint> &keypoints, cv::Mat &descriptors)
{
    ...
    //std::cout<<"desc squeeze size: " << desc.sizes() << std::endl;
    ...
    //std::cout<<"desc transpose size: " << desc.sizes() << std::endl;

    cv::Mat desc_mat(cv::Size(desc.size(1), desc.size(0)), CV_32FC1, desc.data<float>());

    //std::cout<<"desc_mat size: " << desc_mat.size() << std::endl;

    descriptors = desc_mat.clone();

    //std::cout<<"descriptors size: " << desc.sizes() << std::endl;
}
